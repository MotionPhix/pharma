<script setup lang="ts">
import { ref, onMounted } from 'vue';
import axios from 'axios';
import AppLayout from '@/Layouts/AppLayout.vue';
import PageTitle from '@/Components/PageTitle.vue';
import PharmacyCard from '@/Pages/Pharmacy/Partials/PharmacyCard.vue';
import PathRouteMap from '@/Components/map/PathRouteMap.vue';

const props = defineProps<{
  favorites: Array<{}>
  recommendations: Array<{}>
}>()

const favoritePharmacies = ref([]);
const error = ref(null);

const removeFavorite = async (favoriteId) => {
    try {
        await axios.delete(route('api.favorites.destroy', { id: favoriteId }));
        favoritePharmacies.value = favoritePharmacies.value.filter(pharmacy => pharmacy.favoriteId !== favoriteId);
    } catch (err) {
        console.error('Failed to remove favorite:', err);
    }
};

onMounted(() => {
   favoritePharmacies.value = props.favorites
});
</script>

<template>
  <AppLayout>

  <div class="mx-4 my-12 favorites-page">

    <PageTitle heading="My Favorite Pharmacies" />

    <div v-if="$page.props.status" class="text-center text-red-500">
      {{ $page.props.status }}
    </div>

    <PathRouteMap />

    <div
      v-if="!favoritePharmacies.length"
      class="flex flex-col items-center justify-center gap-4">
      <span>
        <svg
          class="w-24 h-24 stroke-[0.05rem]"
            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none">
            <path d="M20 22V19C20 16.1716 20 14.7574 19.1213 13.8787C18.2426 13 16.8284 13 14 13L12 15L10 13C7.17157 13 5.75736 13 4.87868 13.8787C4 14.7574 4 16.1716 4 19V22" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M16 13V18.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M8.5 13V17M8.5 17C9.60457 17 10.5 17.8954 10.5 19V20M8.5 17C7.39543 17 6.5 17.8954 6.5 19V20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M15.5 6.5V5.5C15.5 3.567 13.933 2 12 2C10.067 2 8.5 3.567 8.5 5.5V6.5C8.5 8.433 10.067 10 12 10C13.933 10 15.5 8.433 15.5 6.5Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M16.75 19.25C16.75 19.6642 16.4142 20 16 20C15.5858 20 15.25 19.6642 15.25 19.25C15.25 18.8358 15.5858 18.5 16 18.5C16.4142 18.5 16.75 18.8358 16.75 19.25Z" stroke="currentColor" stroke-width="1.5" />
          </svg>
      </span>

      <span class="text-center">
        You have no favorites yet. Here are some pharmacies you might like:
      </span>

      <div
        class="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 lg:grid-cols-3">
        <PharmacyCard
          v-for="pharmacy in recommendations"
          :key="pharmacy.id"
          :pharmacy="pharmacy"
        />
      </div>
    </div>

    <div v-else class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      <div
        v-for="pharmacy in favoritePharmacies"
        :key="pharmacy.id"
        class="relative"
      >
        <PharmacyCard :pharmacy="pharmacy" />

        <button
          @click="removeFavorite(pharmacy.favoriteId)"
          class="absolute px-2 py-1 text-white bg-red-500 rounded-full top-2 right-2"
        >
          Remove
        </button>
      </div>
    </div>
  </div>
</AppLayout>
</template>

<style scoped>
.favorites-page {
  @apply max-w-3xl sm:mx-auto
}
</style>
